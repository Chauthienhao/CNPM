      ...
      <div className="Main">
        <div className="thongbao-container">
          ...
          <div className="notification-actions">
            <button className="add-btn" onClick={() => setShowAddModal(true)}>Thêm thông báo</button>
          </div>
            ...
            {notifications.map(note => (
              <div className="notification-card" key={note.id}>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '10px' }}>
                  <button className="edit-btn" onClick={() => handleEdit(note)}>Sửa</button>
                  <button className="delete-btn" onClick={() => handleDelete(note.id)}>Xóa</button>
                </div>
              </div>
            ))}
          </div>
            ...
            <div className="modal-overlay">
              <div className="modal-content">
                ...
                <select
                  ...
                  onChange={e => setNewNote({ ...newNote, parent_id: e.target.value })}
                >
                  <option value="">--Chọn phụ huynh--</option>
                  {parents.map(p => (
                    <option key={p.id} value={p.id}>{p.ho_ten}</option>
                  ))}
                </select>
                ...
                <div className="modal-actions">
                  <button className="add-btn" onClick={handleSubmitAdd}>Thêm</button>
          ...
          {showEditModal && (
            <div className="modal-overlay">
              <div className="modal-content">
                ...
                <div className="modal-actions">
                  <button className="add-btn" onClick={handleSubmitEdit}>Lưu</button>
          ...
          <div className="notification-actions">
            <button className="send-btn" onClick={handleSendAll}>Gửi thông báo</button>
          </div>
        </div>
      </div>
    </div>
